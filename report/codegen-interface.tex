...
(\ttt{check}), type deductie (\ttt{infer}), het uitvoerformaat (\ttt{format}),
waar de uitvoer moet geplaatst worden (\ttt{output}), welke taal gebruikt moet
worden (\ttt{language}) en voor welk platform de generatie moet gebeuren
(\ttt{platform}).

De lijst van mogelijke uitvoerformaten bestaat uit: \ttt{none}, \ttt{ast},
\ttt{ast-dot}, \ttt{sm-dot}, \ttt{foo} en \ttt{code}.

Formaat \ttt{ast} toont een hi\"erarchisch overzicht van de AST op het scherm
in tekstuele vorm. De uitvoer van \ttt{ast-dot} zagen we in essentie reeds
...
deze AST met een boomstructuur. Door middel van het \ttt{dot} commando kan
...
De verschillende fasen uit het generatie proces bestaan uit het aanmaken van
...
genereren van de code. De bijkomende \ttt{load} functie combineert de
\ttt{create\_model} en \ttt{parse} functionaliteit in \'e\'en handige functie.
